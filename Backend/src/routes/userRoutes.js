...
router.get('/user_wallets', ClerkExpressRequireAuth(), async (req, res) => {
    ...
    try {
        ...
        console.log("Schritt 1: Clerk User ID aus Token extrahiert:", clerkUserId);

        if (!clerkUserId) {
            console.error("Fehler: Keine Clerk User ID im Token gefunden.");
            return res.status(401).json({ error: "Authentifizierung fehlgeschlagen." });
        }
        console.log(`Schritt 2: Suche in der Datenbank nach Wallet für User: ${clerkUserId}`);
